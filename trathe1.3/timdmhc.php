...
if($_POST['dmhc']){
	...
	$user=$_SESSION['user'];
	$db->query("INSERT INTO `lichsu` (`id`, `time`, `action`, `tongso`, `timthay`, `khongthay`, `user`) VALUES (NULL, current_timestamp(), 'timmahc', '".(count($mangten)-1)."', '', '', '".$user."')");
	$db->query("update login set luottimmahc=luottimmahc+1 where user='".$user."'");
	echo "</table>";
