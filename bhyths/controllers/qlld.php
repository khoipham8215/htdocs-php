...
session_start();
	if(!empty($_SESSION['user'])){
		...
		if(isset($kq)){
			$stt=1;
			echo "<div class='container-fluid kqt'>Danh sách học sinh đang tham gia BHYT tại trường</div>";
			echo "<div class='container-fluid tbhs'><table class='table table-hover'><tr class='ttr'><th>STT</th><th>Mã đơn vị</th><th>Số sổ BHXH</th><th>Mã thẻ BHYT</th><th>Mã nơi KCB</th><th>Họ tên</th><th>Ngày Sinh</th><th>Giới tính</th><th>Mức Lương</th><th>Hệ số lương</th><th>Số CMND</th><th>Lớp</th><th>Cập nhật số điện thoại</th></tr>";
		foreach($kq as $k){
			echo "<tr><td>".$stt."</td><td>".$k['maDvi']."</td><td>".$k['soBhxh']."</td><td>".$k['soKcb']."</td><td>".$k['maBv']."</td><td>".$k['hoTen']."</td><td>".$k['ngaySinh']."</td><td>".$k['gioiTinh']."</td><td>".$k['mucLuong']."</td><td>".$k['heSoLuong']."</td><td>".$k['soCmnd']."</td><td>".$k['maPb']."</td><td><input class='sdt' name='".$k['soBhxh']."' type='text' onchange='kiemtrasdt(this)' /></td></tr>";
			$stt++;
			}
