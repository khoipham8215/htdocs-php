...
if($_POST['loaitc']){
	...
	for($i=0;$i<(count($dltra)-1);$i++){
		if($dltra!=""){
			switch($_POST['loaitc']){
				...
				break;
				case 5 : $kq=tratheBHYT5($dltra[$i],$manghs);
				break;
	...
	echo "<p class ='td'> Danh sách tổng hợp học sinh năm ".$_POST['namps']." </p>";
	echo "<table class='tbhs'><tr><th>STT</th><th>Mã rà soát</th><th>Mã đơn vị cấp thẻ</th><th>Số BHXH</th><th>Mã thẻ</th><th>Họ tên</th><th>Ngày Sinh</th><th>Hạn thẻ từ ngày</th><th>Hạn thẻ đến ngày</th><th>Ngày cấp</th><th>Địa chỉ</th></tr>";
	$j=0;
	$k=0;
	for($i=0;$i<count($dltra)-1;$i++){
		if($i===$mangkt[$j]){
			...
			$marasoat=$dl[2];
			$j++;
			echo "<tr class='kt'><td>".($i+1)."</td><td>".$marasoat."</td><td> Đối tượng không thẻ </td><td></td><td></td><td>".$hoten."</td><td>".$ngaysinh."</td><td></td><td></td><td></td><td></td></tr>";
			...
			$dl=explode("#",$dltra[$i]);
			$marasoat=$dl[2];
			echo "<tr ><td>".($i+1)."</td><td>".$marasoat."</td><td>".$manghs[$id]['maDonViThu']."</td><td>".($manghs[$id]['soBhxh'])."</td><td>".($manghs[$id]['soKcb'])."</td><td>".($manghs[$id]['hoTen'])."</td><td>".($manghs[$id]['ngaySinh'])."</td><td>".($manghs[$id]['tuNgay'])."</td><td>".($manghs[$id]['denNgay'])."</td><td>".($manghs[$id]['ngayCap'])."</td><td>".($manghs[$id]['diaChiLh'])."</td></tr>";
		}
	}
	/**
